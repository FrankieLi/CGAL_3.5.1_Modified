...
operator>>(std::istream &is, Mesh_surface_cell_base_3<GT, MT, Cb> &c)
{
  ...
  for(int i = 0; i < 4; ++i)
  {
    Surface_index index;
    if(is_ascii(is))
      is >> index.first >> index.second;
    else
    {
      read(is, index.first);
      read(is, index.second);
    }
           ...
           const Mesh_surface_cell_base_3<GT, MT, Cb> &c)
{
  ...
  for(int i = 0; i < 4; ++i)
  {
    const Surface_index& s_index = c.surface_index(i);
    if(is_ascii(os)) {
      os << ' ' << s_index.first << ' ' << s_index.second;
    }
    else {
      write(os, s_index.first);
      write(os, s_index.second);
    }
  }
  return os;
}
